...
buttonStart.addEventListener("click", ()=>{
    if (buttonStart.className === "button restart") game.restart();
    game.start();
    updateView();
...
class Game {
    ...
    hasEmptyCells() {
        for(let row = 0; row < 4; row++)for(let column = 0; column < 4; column++){
            if (this.state[row][column] === 0) return true;
        }
        return false;
    }
    canCombine() {
        for(let row = 0; row < 4; row++)for(let column = 0; column < 4; column++){
            ...
            if (column < 3 && current === this.state[row][column + 1]) return true;
            if (row < 3 && current === this.state[row + 1][column]) return true;
        }
        return false;
    }
    checkGameState() {
        for(let row = 0; row < 4; row++){
            for(let column = 0; column < 4; column++)if (this.state[row][column] === 2048) {
                this.status = "win";
                return;
            }
        }
    ...
    transposeState(state) {
        ...
        for(let column = 0; column < 4; column++){
            result[column] = [];
            for(let row = 0; row < 4; row++)result[column].push(state[row][column]);
        }
        return result;
    }
    areStatesEqual(state1, state2) {
        for(let row = 0; row < 4; row++)for(let column = 0; column < 4; column++){
            if (state1[row][column] !== state2[row][column]) return false;
        }
        return true;
    }
}
